...
## ADR 04: Persistent Favorites Manager and Autocompleter
* **Date**: 2026-06-19

### Context
Users need a way to persistently save favorite prompt templates (surviving browser restarts and tab closures), manage them inside the sidebar, quickly add existing prompts to favorites, and easily autocomplete/reuse them inside ChatGPT's text input box.

### Decision
1. **Persistent Storage**: Use `chrome.storage.local` to store favorites under the key `chatToc:favorites` (re-adding `"storage"` permission in the manifest).
2. **Sorting & Filtering**: Provide 4 sorting filters (Alphabetical A-Z/Z-A, Update Time Asc/Desc) inside the Favorites view.
3. **Right-Click Quick Add**: Intercept `contextmenu` events on the TOC list item and directly open the Create Favorite modal pre-filled with the prompt's content, avoiding UI clutter from redundant hover buttons.
4. **Autocomplete Overlay**: Listen to the `input` event on ChatGPT's `#prompt-textarea`. Trigger autocomplete overlays on a slash command (`/`) or when matching favorite titles, and insert contents using `document.execCommand('insertText')` to integrate with React's state management.
...
* Storing favorites in `chrome.storage.local` matches the expectation of a permanent user-defined database, unlike session-bound states.

### Consequences
* `"storage"` permission was restored in `manifest.json`.
* New file `favorites.js` was introduced to isolate favorites management and keep content.js focused on TOC layout.
